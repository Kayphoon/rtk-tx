...
# Use local release build if available, otherwise fall back to installed rtk-tx
if [ -f "./target/release/rtk-tx" ]; then
  RTK="$(cd "$(dirname ./target/release/rtk-tx)" && pwd)/$(basename ./target/release/rtk-tx)"
elif command -v rtk-tx &> /dev/null; then
  RTK="$(command -v rtk-tx)"
else
  echo "Error: rtk-tx not found. Run 'cargo build --release' or install rtk-tx."
  exit 1
fi
...
bench_rewrite "rewrite quoted"       "$RTK rewrite 'git status'"     "rtk-tx git status"
bench_rewrite "rewrite unquoted"     "$RTK rewrite git status"       "rtk-tx git status"
bench_rewrite "rewrite ls -al"       "$RTK rewrite ls -al"           "rtk-tx ls -al"
bench_rewrite "rewrite npm exec"     "$RTK rewrite npm exec"         "rtk-tx npm exec"
bench_rewrite "rewrite cargo test"   "$RTK rewrite cargo test"       "rtk-tx cargo test"
bench_rewrite "rewrite compound"     "$RTK rewrite 'cargo test && git push'" "rtk-tx cargo test && rtk-tx git push"
