#!/usr/bin/env bash
# Install rtk-tx from a local release build (builds from source, no network download).

set -euo pipefail

INSTALL_DIR="${1:-$HOME/.cargo/bin}"
INSTALL_PATH="${INSTALL_DIR}/rtk-tx"
BINARY_PATH="./target/release/rtk-tx"
